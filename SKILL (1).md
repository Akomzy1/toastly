...
| File | Covers |
|---|---|
| `Toastly_Design_System.html` | **Read first.** Colour tokens, type scale, component specs |
| `Toastly_Home__1_.html` | Home / landing |
| `Toastly_Features__1_.html` | Features (six deep-dive sections) |
| `Toastly_How_It_Works__1_.html` | Six-step journey |
| `Toastly_Pricing.html` | Nigeria (₦) and Diaspora ($) tracks, coins |
| `Toastly_Safety.html` | Safety & trust |
| `Toastly_Diaspora__1_.html` | Diaspora — both matching pools |
| `Toastly_Stories__1_.html` | Testimonials, gallery, community |
| `Toastly_Locked_Inbox.html` | **In-app** locked inbox state (not marketing) |
| `Toastly_Brand_-_The_Stake.html` | **Adopted brand mark ("The Stake") — binding.** Mark geometry, palette roles, lockups, do/don't |
...
`Toastly_Brand_-_The_Stake.html` is binding. Two overlapping coin rings above a line, derived from the coin-deposit mechanic — the overlap is two people meeting halfway, the line is the table they both showed up to.
...
1. **Home page** claimed Couple Mode + AriyaPlanner are "included on Premium Plus." **Wrong** — they are free on every tier including Starter, as `Toastly_Pricing.html` correctly shows. Build the free version.
...
- Missing that `Toastly_Locked_Inbox.html` is an **in-app** screen, not a marketing page — it has different layout rules.
