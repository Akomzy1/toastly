...
**The design-system file defines the full ramp (22 steps) — all of it is approved.** Encode the whole ramp, and **replace `theme.colors` rather than extending it**, so an unapproved colour fails at build time. **Do not build a dark theme:** deep green is a ground, not a mode; leave any `.dark` values provisional and add no `dark:` variants.

Also read `/design/prototype/Toastly_Brand_-_The_Stake.html` — the adopted brand mark, with fixed geometry and assigned palette roles. Treat it as binding alongside the design system.
